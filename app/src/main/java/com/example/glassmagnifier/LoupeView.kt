...
//    private var loupeRadius: Int
    ...
    fun setMagnificationFactor(factor: Float) {
        // Adjust the magnification factor within reasonable limits
        magnificationFactor = if (factor > 0) factor else 1.0f
        invalidate() // Trigger a redraw
    }

    fun setLoupeRadius(radius: Int) {
        // Adjust the loupe radius within reasonable limits
        loupeRadius = if (radius > 0) radius else 1
        invalidate() // Trigger a redraw
    }
